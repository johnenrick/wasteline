...
<script>
    ...
    profileManagement.viewProfile = function () {
        $.post(api_url("c_account/retrieveAccount"), {ID: user_id()}, function (data) {
            ...
            if (!response["error"].length) {
                ...
                if (typeof profileManagement.webMap !== "undefined" && response["data"]["account_address_map_marker_ID"]) {
                    ...
                    setTimeout(function () {
                        if (!$("#wl-btn-editProfile").is(":visible")) {
    ...
    profileManagement.initializeWebMap = function () {
        if (typeof profileManagement.webMap === "undefined") {
            ...
            profileManagement.webMap.tileLayer.on("load", function () {
                ...
                var lat  = $("#profileManagementForm").find("[name='updated_data[account_address_latitude]']").val();
                var lng = $("#profileManagementForm").find("[name='updated_data[account_address_longitude]']").val();
                if (($("#profileManagementForm").find("[name='updated_data[account_address_latitude]']").val() === "") && ($("#profileManagementForm").find("[name='updated_data[account_address_longitude]']").val() === "")) {
                    lng = 123.922587;
                    lat = 10.339634;
                    $("#profileManagementForm").find("[name='updated_data[account_address_latitude]']").val(lat);
                    $("#profileManagementForm").find("[name='updated_data[account_address_longitude]']").val(lng);
                }
                profileManagement.changeAddress({
                    lat: lat,
                    lng: lng
    ...
    $(document).ready(function () {
        load_page_component("web_map_component", function () {
        });
